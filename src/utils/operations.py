...
class Hikvision:
    ...
    def getsec(self):         
        ...
        rtsp_req = ISAPI(f'http://{self.ip}/ISAPI/Streaming/channels/101', self.password)
    ...
    def getdns(self):
        ...
        return {'primary': primary_dns_text, 
                'secondary': secondary_dns_text}
    ...
    def getmail(self):
        ...
        if receiver_lst_tag is not None:
            ...
            if receiver_tag is not None:
                ...
                receiver_email_tag = receiver_tag.find('ns:emailAddress', self.namespace)
    ...
    #GET OSD Name
    def getosd(self):
        req = ISAPI(f'http://{self.ip}/ISAPI/System/Video/inputs/channels/1', self.password)
        response = str(req.getapi())
        root = ET.ElementTree(ET.fromstring(response))
        name_text = None
    ...
    def getoverlay(self):
        req = ISAPI(f'http://{self.ip}/ISAPI/System/Video/inputs/channels/1/overlays', self.password)
        response = str(req.getapi())
        root = ET.ElementTree(ET.fromstring(response)).getroot()
        week_text = None
        format_text = None
        ...
        return {'week': week_text,
                'format': format_text}

    #GET Motion Detection
    def getmotion(self):
        req = ISAPI(f'http://{self.ip}/ISAPI/System/Video/inputs/channels/1/motionDetection', self.password)
        response = str(req.getapi())
        root = ET.ElementTree(ET.fromstring(response)).getroot()
        enabled_text = None
        grid_text = None
        ...
        for track_tag in root.findall('.//ns:Track', self.namespace):
            ...
            if not custom_flag:
                ...
                if custom_ext_lst is not None:
                    custom_ext = custom_ext_lst.find('ns:CustomExtension', self.namespace)
                    if custom_ext is not None:
                        #Enabled Scheduler
                        enabled_tag = custom_ext.find('ns:enableSchedule', self.namespace)
                        if enabled_tag is not None:
                            enabled_text = enabled_tag.text
                        #Pre-Record seconds
                        pre_tag = custom_ext.find('ns:PreRecordTimeSeconds', self.namespace)
                        if pre_tag is not None:
                            pre_text = pre_tag.text
                        #Post-Record seconds
                        post_tag = custom_ext.find('ns:PostRecordTimeSeconds', self.namespace)
            ...
            if track_sch_tag is not None:
                block_list_tag = track_sch_tag.find('ns:ScheduleBlockList', self.namespace)
                if block_list_tag is not None:
                    block_tag = block_list_tag.find('ns:ScheduleBlock', self.namespace)
                    if block_tag is not None:
                        for sch_action_tag in block_tag.findall('ns:ScheduleAction', self.namespace):
                            #Start Time
                            start_tag = sch_action_tag.find('ns:ScheduleActionStartTime', self.namespace)
                            if start_tag is not None:
                                ...
                                #Start Hour
                                time_tag = start_tag.find('ns:TimeOfDay', self.namespace)
                                if time_tag is not None:
                                    start_text = time_tag.text
                                    start_list.append(start_text)

                            #End Time
                            end_tag = sch_action_tag.find('ns:ScheduleActionEndTime', self.namespace)
                            ...
                            #Action
                            action_tag = sch_action_tag.find('ns:Actions', self.namespace)
                            if action_tag is not None:
                                mode_tag = action_tag.find('ns:ActionRecordingMode', self.namespace)
                                if mode_tag is not None:
                                    mode_text = mode_tag.text
                                    mode_list.append(mode_text)
  
        return {'enabled': enabled_text,
                'advanced': {'overwrite': over_text,
                             'prerecord': pre_text,
                             'postrecord': post_text,
                             'expiration': expiration_text,
                             'duration': duration_text},
                'schedule': {'start': dict(start_dict), 
                             'end': dict(end_dict), 
                             'mode': dict(mode_dict)}}
